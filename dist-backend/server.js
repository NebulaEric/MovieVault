...
app.get('/api/media', (req, res) => {
    ...
    try {
        if (filter === "all") {
            ...
            rows = datab.prepare(`SELECT * FROM media ORDER BY id DESC`).all();
        }
        else {
            console.log("this is the filter33: ", filter);
            rows = datab.prepare(`SELECT * FROM media WHERE media_type = ? ORDER BY id DESC`).all(filter);
        }
...
app.post('/api/movies', async (req, res) => {
    ...
    try {
        const transaction = datab.transaction(() => {
            const info = insertMediaStmt.run(movie.media_type, movie.title, movie.year, movie.poster);
            const otherInfo = insertMovieStmt.run(movie.tmdb_id, info.lastInsertRowid, movie.backdrop, movie.overview);
...
app.post('/api/tv', async (req, res) => {
    ...
    try {
        const transaction = datab.transaction(() => {
            const info = insertMediaStmt.run(tv.media_type, tv.title, tv.year, tv.poster);
            const tvInfo = insertTVStmt.run(tv.tmdb_id, info.lastInsertRowid, tv.backdrop, tv.overview, tv.num_season, tv.num_episode);
            for (const season of seasons) {
                insertSeasonStmt.run(season.id, season.air_date, season.episode_count, season.name, season.overview, season.season_number, season.poster);
            }
            id = info.lastInsertRowid;
            for (const actor of actors) {
                insertActorStmt.run(actor.id, actor.name, actor.profile_path);
...
//Gets movie data for movie preview page
app.get('/api/preview-show', async (req, res) => {
    const id = req.query.id;
    if (!id) {
        res.status(400).json({ error: 'Missing movie ID' });
        return;
...
app.post('/api/review', async (req, res) => {
    ...
    try {
        const stmt = datab.prepare('INSERT INTO review (profile_id, media_id, rating, comment) VALUES (?, ?, ?, ?)');
...
app.post('/api/watchlist', async (req, res) => {
    ...
    try {
        const stmt = datab.prepare('INSERT INTO watchlist (profile_id, media_id) VALUES (?, ?)');
...
app.get('/api/preview-person', async (req, res) => {
    ...
    try {
        // console.log('Hello1 + ');
        let data;
        data = await datab.prepare('SELECT * FROM actor WHERE id = ?').get(person_id);
        // const results = data
...
app.get('/api/person-movies', async (req, res) => {
    ...
    try {
        ...
        const localMovies = datab.prepare('SELECT m.tmdb_id, md.poster FROM movie m JOIN media md ON m.media_id = md.id;').all();
