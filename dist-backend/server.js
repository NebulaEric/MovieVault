...
app.post('/api/movies', async (req, res) => {
    ...
    try {
        ...
        const transaction = datab.transaction(() => {
            insertMovieStmt.run(movie.tmdb_id, movie.title, movie.year, movie.poster, movie.backdrop, movie.overview);
...
app.get('/api/movies', (req, res) => {
    try {
        const rows = datab.prepare('SELECT * FROM movies ORDER BY id DESC').all();
...
app.get('/api/preview-movie/local', async (req, res) => {
    ...
    try {
        const movie = datab.prepare(`SELECT tmdb_id, title, year, overview, poster, backdrop FROM movies WHERE tmdb_id = ?`).get(id);
...
app.post('/api/watchlist', async (req, res) => {
    ...
    try {
        const stmt = datab.prepare('INSERT INTO watchlist (tmdb_id, title, year, poster) VALUES (?, ?, ?, ?)');
...
app.get('/api/preview-person', async (req, res) => {
    ...
    try {
        // console.log('Hello1 + ');
        let data;
        data = await datab.prepare('SELECT * FROM actors WHERE id = ?').get(person_id);
        // const results = data
...
app.get('/api/person-movies', async (req, res) => {
    ...
    try {
        ...
        const localMovies = datab.prepare('SELECT tmdb_id, poster FROM movies').all();
